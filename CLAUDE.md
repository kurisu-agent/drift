...
Every PR must land with a fresh `flake.lock`. Run `nix flake update` as part of the PR; if it produces a diff, include the bump as its own commit (or fold into the feature commit when the PR is small). This keeps drift's pinned nixpkgs / flake-utils current so each release tag ships against recent inputs and the lock never drifts weeks behind by accumulation.

The rule is enforced at release time by `.github/workflows/release.yml`'s `lock-fresh` job, which runs `nix flake update` on the tagged commit and fails goreleaser if the lock would change. That blocks the *release artifacts*, not the tag itself, but a bare tag with no release is a no-op for users. Because every PR bumps the lock, by the time you tag, main is already fresh and the gate passes without ceremony.

Practical flow at the start of every PR:

```
nix flake update
git add flake.lock
git commit -m "flake.lock: bump for <feature>"
```

If `nix flake update` produces no diff (the lock is already at the latest registry rev), there's nothing to commit. Either way, the gate at release time confirms the state.
...
Versioning until further notice: every release ships under `v0.1.N`. Don't bump the minor or major. The 0.1.x line is the active development train; if a change is large enough to feel like a minor bump, open a discussion first rather than tagging it that way unilaterally.
...
# Testing branch builds end-to-end

When a PR needs a real-system smoke test on a nix workstation beyond what `make ci` covers, pin the system flake's `drift` input at the feature worktree for one rebuild. Same shape for both client (`drift`) and server (`lakitu`) PRs because the nix-config flake imports `drift` as a single input and pulls both packages from `drift.packages.${system}.*`; one swap rebuilds both ends in lockstep.

```
git worktree add .claude/worktrees/<feature> -b feat/<feature> origin/feat/<feature>
sudo nixos-rebuild switch --flake ~/Code/nix-config#<host> \
  --override-input drift path:$HOME/Code/drift/.claude/worktrees/<feature> \
  --impure
```

`--override-input drift path:<absolute-worktree-path>` swaps the locked `github:kurisu-agent/drift` flake input for a path-flake build of the worktree. The nix-config's existing `driftPkg = drift.packages.x86_64-linux.drift` and the parallel lakitu wiring consume the override unchanged, so no edit to `flake.nix` is needed.

`--impure` is required when the host config pulls in a gitignored `local.nix` (the dev VM does this to layer on host-local tweaks like `services.lakitu.nixCache.enable`). Without `--impure`, `builtins.pathExists /etc/nixos/...local.nix` fails the pure-eval gate and the rebuild evaluates *without* the overlay — usually visible as services from local.nix silently disappearing (lakitu missing from `/run/current-system/sw/bin/`, containers torn down, harmonia/nginx removed). The path-flake override on its own is pure; the impurity comes from the host config's overlay imports, not from the override.

Verify the override took:

- Client: `readlink -f "$(which drift)"` should resolve to a fresh `/nix/store/...-drift-dev/bin/drift`. `grep -ao '<some-new-string>' "$(which drift)"` should hit a flag or identifier the branch added.
- Server: `lakitu version` should report the worktree's commit. `systemctl restart lakitu` if you're swapping a long-running service rather than just probing the binary.

Revert by rebuilding without the flag: `sudo nixos-rebuild switch --flake ~/Code/nix-config#<host>`. The system goes back to whatever `flake.lock` pins.

When the branch changes the JSON-RPC schema (new method, new field, renamed type), this approach rebuilds client and server simultaneously, so old-vs-new mismatches don't happen by construction.

For non-nix targets (laptop, generic linux, Termux), `drift update <host>:/path` continues to work — the read-only-store problem is nix-specific. See "Building drift binaries by hand" for the static-build recipe to ship over.
...
The same rule applies with full force to **personal identifiers**: real names, real emails (including `<id>+<handle>@users.noreply.github.com` forms), real GitHub usernames, real character names from the user's chest. None of these belong in any tracked file (test fixtures, code comments, docstrings, plans, TODO.md, README) or in any commit message, PR title, or PR body. Use `Example User` / `user@example.test` / `example-user` / `your-org/your-repo` / `placeholder-pat`. Treat every commit and PR field as if it will be public the moment it leaves the local clone — because it will be, and force-pushes don't undo GitHub's events API or anyone who already cloned. If a leak slips through, surface it to the user before running cleanup; undirected destructive recovery (force-push, branch delete, repo recreate) compounds the damage.

When tracing a real bug on the user's circuit, describe the *shape* of the problem ("a PAT-less character inherited the host gitconfig") rather than the actors ("character `foo` inherited `bar`'s identity"). Local state (`~/.drift/garage/characters/*.yaml`, dechested PATs, etc.) is allowed to inform diagnosis in conversation but must be sanitized to placeholders before any of it touches a tracked file or pushed message.
