...
// `--extra-devcontainer-path` on devpod up. With drift's default kart
// runArgs (IPv6-disable sysctl) every overlay is now non-empty, so the
// staged file is read, spliced, and re-written under tmpDir — the path
// devpod sees is the spliced artifact, not the user's input. The
// project's image declaration is preserved verbatim and the runArgs
// are appended.
func TestTuneDevcontainer(t *testing.T) {
	...
	// splice path has a real file to read from.
	...
	if got == dcPath {
		t.Errorf("--extra-devcontainer-path should be the spliced artifact, not the staged input %q", dcPath)
	}
	if got == "" {
		t.Fatalf("--extra-devcontainer-path missing; argv=%v", up.Argv)
	}

	// Artifact check: the staged image declaration survives the splice
	// and the default runArgs sysctl is appended.
	body := c.ReadArtifact(ctx, up, "extra-devcontainer.json")
	if err := assertJSONEqual(string(body),
		`{"image":"alpine:latest","runArgs":["--sysctl=net.ipv6.conf.all.disable_ipv6=1"]}`); err != nil {
...
func TestFeaturesFlagExplicit(t *testing.T) {
	...
	// --tune none opts out of the server default_tune ("nixenv"), which
