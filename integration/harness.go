...
// FindSSH returns the first `devpod ssh <kart> --command <cmd>` invocation
// in the recorder log. The post-up script (symlinks, seeds, gh-auth) lands
// here as `--command bash -s` with the script body on stdin — read via
// ReadArtifact(inv, "stdin").
func (r *DevpodRecorder) FindSSH(ctx context.Context) *DevpodInvocation {
	for _, inv := range r.Invocations(ctx) {
		if len(inv.Argv) > 0 && inv.Argv[0] == "ssh" {
			cp := inv
			return &cp
		}
	}
