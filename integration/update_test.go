...
func TestDriftUpdate_MissingResolvConf(t *testing.T) {
	...
	// What matters is that the GitHub API responded — that's what
	// proves DNS resolved. `latest:` appears on a 200; `github api
	// returned` appears on any non-2xx (e.g. 404 on a fresh repo
	// before the first release). The regression we're guarding
	// against (ECONNREFUSED from Go picking loopback) produces
	// neither, so accept either as proof DNS engaged.
	s := string(out)
	if !strings.Contains(s, "latest:") && !strings.Contains(s, "github api returned") {
		t.Fatalf("no GitHub API response — DNS fallback did not engage\nargs: %v\noutput:\n%s",
			args, out)
	}
}
