...
// TestTuneEnvBuildInjection verifies env.build entries ride on
// `devpod up --dotfiles-script-env` (so the layer-2 dotfiles install
// script sees the build secret) without leaking into the workspace's
// containerEnv. Covers Stage #1 (one-shot build-time secret).
