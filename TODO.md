...
- Multi-repo workspaces — a single kart hosting more than one cloned repo side-by-side, with tune/starter shape extended to declare the set.
- `drift ports update` — re-sync forwards on demand: auto-reconnect dropped tunnels and rebind ports (e.g. vite's `127.0.0.1` default that breaks remote forwarding) without requiring a full `drift connect` cycle.
