...
// Run pipes the assembled script over stdin to `bash -s` inside the
// kart. Stdin (instead of devpod ssh's --command) is what keeps
// secret-bearing fragments (ghAuthFragment carries the kart's PAT)
// off lakitu's and the container's argv tables. Prelude (strict mode
// + HOME guard) is prepended here, so fragments can assume `$HOME`
// is set and the shell is bash.
func (s *containerScript) Run(ctx context.Context, dp *devpod.Client, kart string) error {
	...
	if _, err := dp.SSH(ctx, devpod.SSHOpts{
		Name:    kart,
		Command: "bash -s",
