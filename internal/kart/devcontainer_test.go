...
// TestSpliceOverlayAddsRunArgsToEmptyBase covers the no-existing-runArgs
// case: drift's defaults land verbatim.
func TestSpliceOverlayAddsRunArgsToEmptyBase(t *testing.T) {
	base := []byte(`{"image":"ubuntu"}`)
	out, err := spliceOverlay(base, Overlay{RunArgs: []string{"--sysctl=net.ipv6.conf.all.disable_ipv6=1"}})
	if err != nil {
		t.Fatal(err)
	}
	var got map[string]any
	...
	args, ok := got["runArgs"].([]any)
	if !ok || len(args) != 1 || args[0] != "--sysctl=net.ipv6.conf.all.disable_ipv6=1" {
		t.Fatalf("runArgs not spliced: %v", got["runArgs"])
	}
}

// TestSpliceOverlayAppendsRunArgs covers the case where the project's
// devcontainer.json already declares runArgs — drift's defaults are
// appended after, project entries preserved.
func TestSpliceOverlayAppendsRunArgs(t *testing.T) {
	base := []byte(`{"runArgs":["--cap-add=SYS_PTRACE","--security-opt=seccomp=unconfined"]}`)
	out, err := spliceOverlay(base, Overlay{RunArgs: []string{"--sysctl=net.ipv6.conf.all.disable_ipv6=1"}})
	if err != nil {
		t.Fatal(err)
	}
	...
	args := got["runArgs"].([]any)
	want := []string{"--cap-add=SYS_PTRACE", "--security-opt=seccomp=unconfined", "--sysctl=net.ipv6.conf.all.disable_ipv6=1"}
	if len(args) != len(want) {
		t.Fatalf("expected %d runArgs, got %d: %v", len(want), len(args), args)
	}
	for i, w := range want {
		if args[i] != w {
			t.Errorf("runArgs[%d] = %v, want %s", i, args[i], w)
		}
	}
}

// TestSpliceOverlayRunArgsDedup proves the overlay drops a duplicate
// entry the project already declared, so `docker run` stays quiet
// instead of carrying repeated `--sysctl` lines.
func TestSpliceOverlayRunArgsDedup(t *testing.T) {
	base := []byte(`{"runArgs":["--sysctl=net.ipv6.conf.all.disable_ipv6=1"]}`)
	out, err := spliceOverlay(base, Overlay{RunArgs: []string{"--sysctl=net.ipv6.conf.all.disable_ipv6=1"}})
	if err != nil {
		t.Fatal(err)
	}
	...
	args := got["runArgs"].([]any)
	if len(args) != 1 {
		t.Fatalf("dedup expected 1 entry, got %d: %v", len(args), args)
	}
}

// TestDefaultKartRunArgsDisablesIPv6 pins drift's default — the
// IPv6-disable sysctl is what gets every kart out from under the
// dual-stack ENETUNREACH trap on IPv4-only circuits.
func TestDefaultKartRunArgsDisablesIPv6(t *testing.T) {
	want := "--sysctl=net.ipv6.conf.all.disable_ipv6=1"
	for _, arg := range DefaultKartRunArgs {
		if arg == want {
			return
		}
	}
	t.Errorf("DefaultKartRunArgs missing %q: %v", want, DefaultKartRunArgs)
}
