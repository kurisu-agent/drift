...
//
// HasPAT carries the resolved character's PAT presence as a non-empty
// string ("true") when set so seed templates can gate gh-auth advice
// on `{{ if .HasPAT }}`. HasDenyLiterals does the same for the
// kart-side deny-list (plan 20). Stored as strings because seed.Vars
// is homogeneous (text/template's truthiness check on `""` vs
// anything-else is exactly what we want).
func kartVars(r *Resolved) seed.Vars {
	...
	hasPAT := ""
	if r.Character != nil && r.Character.PAT != "" {
		hasPAT = "true"
	}
	hasDenyLiterals := ""
	if r.DenyLiterals != "" {
	...
	return seed.Vars{
		...
		"CharacterDisplayName": r.CharacterDisplayName,
		"CharacterIcon":        r.CharacterIcon,
		"CharacterColor":       r.CharacterColor,
		...
		"HasDenyLiterals":      hasDenyLiterals,
	}
}
