...
type Tune struct {
	...
	// FlakeURI, when set, declares the kart's primary Nix flake. Drift
	// transparently wires up everything needed to deliver it: injects
	// the community Nix devcontainer feature (DS installer, daemon-aware
	// at runtime), splices a postCreateCommand that runs
	// `nix profile install <flake_uri>` once with --extra-substituters
	// pointing at the circuit's harmonia (when configured), and lets
	// the standard upstream pubkey ride along so cache-miss falls
	// through cleanly. Single string for v1 — multiple flakes can come
	// later if needed.
	FlakeURI string `yaml:"flake_uri,omitempty" json:"flake_uri,omitempty"`

	// Icon is the default nerd-font glyph for karts created from this
	// tune; copied into KartConfig.Icon at kart.new and rendered in the
	// in-kart zellij topbar via /run/drift/info.json. Per-kart override
	// is just `lakitu kart edit` (or YAML edit) of `icon` on the kart's
	// config.yaml — Icon here is only the default.
	Icon string `yaml:"icon,omitempty" json:"icon,omitempty"`
	// Color mirrors Icon: catppuccin-mocha palette name (`green`,
	// `mauve`, `peach`, `blue`, `red`, ...) defaulted from the tune,
	// overridable per-kart via the kart's own `color`.
...
type KartConfig struct {
	...
	// PATSlug, when set, names the registered fine-grained PAT this kart
	// uses for git auth. Resolved through the registry to a chest entry
	// at every dotfiles-write so chest rotations propagate without
	// rewriting the kart record. Overrides any pat_secret the kart's
	// character would otherwise contribute.
	PATSlug    string `yaml:"pat_slug,omitempty" json:"pat_slug,omitempty"`
	SourceMode string `yaml:"source_mode,omitempty" json:"source_mode,omitempty"`
	User       string `yaml:"user,omitempty" json:"user,omitempty"`
	Shell      string `yaml:"shell,omitempty" json:"shell,omitempty"`
	Image      string `yaml:"image,omitempty" json:"image,omitempty"`
	Workdir    string `yaml:"workdir,omitempty" json:"workdir,omitempty"`
	// Icon is a single nerd-font glyph rendered in the in-kart zellij
	// topbar (and any other display surface that picks up the kart's
	// info.json). Empty string ⇒ no glyph, fall through to bare name.
	Icon string `yaml:"icon,omitempty" json:"icon,omitempty"`
	// Color is a catppuccin-mocha palette name (`green`, `mauve`,
	// `peach`, etc.) used to tint kart-identifying segments in the
	// topbar / prompt. Empty string ⇒ overlay grey.
	Color        string        `yaml:"color,omitempty" json:"color,omitempty"`
...
//
// DisplayName / Icon / Color are pure presentation: rendered in the
// in-kart zellij topbar and in operator-facing CLI surfaces (`drift
// status`, `drift kart info`) wherever the character would otherwise
// appear as the bare YAML key. The CLI keeps the YAML key as the safe
// identifier (flag values, ssh aliases, dotted paths); display fields
// only ever decorate it.
type Character struct {
	...
	// Icon mirrors Tune.Icon: nerd-font catalog name, single grapheme,
	// or emoji. Resolved to the rendered glyph by internal/icons at
	// info.json render time so flake consumers can `jq` it without
	// touching the catalog.
	Icon string `yaml:"icon,omitempty" json:"icon,omitempty"`
	// Color mirrors Tune.Color: catppuccin-mocha palette name (`green`,
	// `mauve`, `peach`, ...). Empty string ⇒ overlay grey downstream.
	Color string `yaml:"color,omitempty" json:"color,omitempty"`
}
