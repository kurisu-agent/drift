...
func TestServer_Validate(t *testing.T) {
	...
	// deny_literals: empty OK; chest:<name> OK; literal value rejected.
	s = config.DefaultServer()
	s.DenyLiterals = ""
	if err := s.Validate(); err != nil {
		t.Errorf("empty deny_literals should validate, got: %v", err)
	}
	s.DenyLiterals = "chest:my-list"
	if err := s.Validate(); err != nil {
		t.Errorf("deny_literals=chest:my-list should validate, got: %v", err)
	}
	s.DenyLiterals = "alpha\nbeta\n"
	if err := s.Validate(); err == nil {
		t.Error("expected error for literal deny_literals (must be chest:<name>)")
	}
	s.DenyLiterals = "chest:"
	if err := s.Validate(); err == nil {
		t.Error("expected error for chest: with empty name")
	}
}
