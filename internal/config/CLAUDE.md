...
- **run** — a named shorthand in `~/.drift/runs.yaml` invoked as `drift run <name> [args…]` from the workstation. `drift runs` lists them.
...
Larger prompts / scripts belong under `~/.drift/recipes/` (append them as a `--append-system-prompt "$(cat recipes/<file>.md)"` fragment on the claude invocation). The registry file is user-editable and `lakitu init` only seeds it on first run; later re-inits never overwrite.

For Claude-specific dispatch, prefer skills over runs: add a new SKILL.md under `~/.claude/skills/<name>/` and it becomes reachable as `drift skill <name> [prompt]` from the workstation without touching `runs.yaml`. The client pre-prefixes each invocation with "Use the `<name>` skill." so the dispatcher picks it up.
