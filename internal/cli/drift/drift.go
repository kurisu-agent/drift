...
type CLI struct {
	...
	Pats     patsCmd     `cmd:"" name:"pats" help:"List GitHub PATs registered on the target circuit."`
	...
	Pat patCmd `cmd:"" name:"pat" help:"Manage GitHub fine-grained PATs registered with the circuit."`
...
func run(ctx context.Context, argv []string, io IO, deps deps) int {
	...
	case "circuit set icon", "circuit set icon <icon>":
		return runCircuitSetIcon(ctx, io, &cli, cli.Circuit_.Set.Icon, deps)
	case "circuit set default", "circuit set default <name>":
		return runCircuitSetDefault(io, &cli, cli.Circuit_.Set.Default, deps)
	case "circuit icons", "circuit icons <query>":
		return runCircuitIcons(io, &cli, cli.Circuit_.Icons)
	...
	case "pats":
		return runPats(ctx, io, &cli, deps)
	case "pat new", "pat new <slug>":
		return runPatNew(ctx, io, &cli, cli.Pat.New, deps)
	case "pat update <slug>":
		return runPatUpdate(ctx, io, &cli, cli.Pat.Update, deps)
	case "pat info <slug>":
		return runPatInfo(ctx, io, &cli, cli.Pat.Info, deps)
	case "pat delete <slug>":
		return runPatDelete(ctx, io, &cli, cli.Pat.Delete, deps)
