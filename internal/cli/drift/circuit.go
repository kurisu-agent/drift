...
type circuitCmd struct {
	...
	Set     circuitSetCmd     `cmd:"" help:"Set a config field on the target circuit (e.g. name, icon)."`
	Icons   circuitIconsCmd   `cmd:"" help:"Browse the shipped nerd-font icon catalog (filterable)."`
...
type circuitSetCmd struct {
	...
	Icon    circuitSetIconCmd    `cmd:"" help:"Set the circuit's icon (nerd-font name like dev-go, or any single emoji)."`
...
type circuitSetIconCmd struct {
	Icon string `arg:"" optional:"" help:"Icon name (e.g. dev-go) or single emoji. Pass empty string to clear; omit for an interactive picker."`
}
...
type circuitIconsCmd struct {
	Query string `arg:"" optional:"" help:"Substring or fuzzy query (e.g. 'rocket', 'go', 'cat'). Empty lists the first --limit names."`
	Limit int    `name:"limit" default:"60" help:"Max rows to print. 0 means no cap."`
}
...
// runCircuitSetIcon mirrors runCircuitSetName for the Icon field. The
// server is the source of truth, so we validate locally for fast UX
// feedback then push to config.set; nothing else needs invalidating
// (icon doesn't appear in ssh aliases or ports.yaml). Calling with an
// empty arg clears the icon.
func runCircuitSetIcon(ctx context.Context, io IO, root *CLI, cmd circuitSetIconCmd, deps deps) int {
	_, circuit, err := resolveCircuit(root, deps)
	if err != nil {
		return errfmt.Emit(io.Stderr, err)
	}

	// Bare `drift circuit set icon` (no arg) prints the current value
	// plus a hint at the browse command — saves a round-trip when users
	// just want to see what's set. The optional `arg:""` tag means an
	// empty Icon string is ambiguous between "not provided" and "clear
	// it"; we resolve that by treating empty-on-call as a read.
	if cmd.Icon == "" {
		var info wire.ServerInfo
		if err := deps.call(ctx, circuit, wire.MethodServerInfo, struct{}{}, &info); err != nil {
			return errfmt.Emit(io.Stderr, err)
		}
		if root.Output == "json" {
			return emitJSON(io, struct {
			...
			}{Circuit: circuit, Icon: info.Icon, Glyph: icons.Resolve(info.Icon)})
		}
		if info.Icon == "" {
			fmt.Fprintf(io.Stdout, "circuit %q has no icon set — try `drift circuit icons <query>` to browse\n", circuit)
		} else {
			fmt.Fprintf(io.Stdout, "circuit %q icon: %s  (%s)\n", circuit, icons.Resolve(info.Icon), info.Icon)
		}
		return 0
	}

	if err := icons.Validate(cmd.Icon); err != nil {
		return errfmt.Emit(io.Stderr, rpcerr.UserError(rpcerr.TypeInvalidFlag, "%v", err))
	}

	if err := deps.call(ctx, circuit, wire.MethodConfigSet,
		map[string]string{"key": "icon", "value": cmd.Icon}, nil); err != nil {
		return errfmt.Emit(io.Stderr, err)
	}

	rendered := icons.Resolve(cmd.Icon)
	if root.Output == "json" {
		return emitJSON(io, struct {
			Circuit string `json:"circuit"`
			Icon    string `json:"icon"`
			Glyph   string `json:"glyph"`
		}{Circuit: circuit, Icon: cmd.Icon, Glyph: rendered})
	}
	p := style.For(io.Stdout, false)
	fmt.Fprintf(io.Stdout, "circuit %s icon → %s  (%s)\n", p.Accent(circuit), rendered, cmd.Icon)
	return 0
}

// runCircuitIcons prints a table of nerd-font icon names matching the
// query, sorted by best-match. JSON output emits the raw list so scripts
// can pipe through jq.
func runCircuitIcons(io IO, root *CLI, cmd circuitIconsCmd) int {
	hits := icons.Search(cmd.Query, cmd.Limit)
	if root.Output == "json" {
		type row struct {
			Name  string `json:"name"`
			Glyph string `json:"glyph"`
		...
		for i, n := range hits {
			r, _ := icons.Lookup(n)
			rows[i] = row{Name: n, Glyph: string(r)}
		}
		return emitJSON(io, struct {
			Query string `json:"query"`
			Total int    `json:"total"`
			Hits  []row  `json:"hits"`
		}{Query: cmd.Query, Total: len(rows), Hits: rows})
	}

	if len(hits) == 0 {
		fmt.Fprintf(io.Stdout, "no matches for %q (try a shorter query)\n", cmd.Query)
		return 0
	}
	p := style.For(io.Stdout, false)
	...
	for _, n := range hits {
		r, _ := icons.Lookup(n)
		rows = append(rows, []string{string(r), n})
	}
	writeTable(io.Stdout, p, []string{"GLYPH", "NAME"}, rows, accentCellStyler(1))
	if cmd.Limit > 0 && len(hits) >= cmd.Limit {
		fmt.Fprintf(io.Stdout, "\n%s\n", p.Dim(
			fmt.Sprintf("(showing %d — pass --limit=0 for the full list)", cmd.Limit)))
	}
	return 0
}
