...
type newCmd struct {
	...
	// Pat names a registered PAT slug (from `drift pats`) to attach to
	// this kart, or the literal `none` to suppress any character-inherited
	// PAT. Raw `github_pat_…` / `ghp_…` literals are rejected here — users
	// must register through `drift pat new` first.
	Pat       string `name:"pat" help:"Use a registered PAT slug for this kart, or 'none' to opt out."`
...
func runNew(ctx context.Context, io IO, root *CLI, cmd newCmd, deps deps) int {
	...
	if err := validatePatFlag(cmd.Pat); err != nil {
		return errfmt.Emit(io.Stderr, err)
	}
	...
	// didn't pass --pat, ask the registry whether any registered PAT
	// covers the target and let the user pick. Skipped silently in
	// non-interactive contexts (scripts, --output json) and when the
	// registry has no matches.
	if cmd.Pat == "" && stdinIsTTY(io.Stdin) && root.Output != "json" {
		if perr := maybePromptForClonePAT(ctx, io, deps, circuit, &cmd); perr != nil {
...
// parseGitHubCloneOwnerRepo extracts `<owner>` and `<repo>` from a github
// clone URL. Returns ok=false for non-github URLs, malformed inputs, or
// anything other than the canonical `github.com/<owner>/<repo>` shape so
// the caller silently falls through (no auto-detect for bitbucket /
// gitlab / self-hosted instances). The trailing `.git` suffix is
// tolerated; query strings and fragments are not (kept conservative on
// purpose — better to miss the picker than misparse it).
func parseGitHubCloneOwnerRepo(url string) (string, string, bool) {
...
// maybePromptForClonePAT runs pat.find_for_clone for the kart's clone
// target and, when matches come back, renders a picker. The user picks
// a slug or selects "skip / no PAT" — the latter leaves cmd.Pat empty,
// which means the kart-create resolver falls through to the character's
// pat_secret (if any). No matches ⇒ no picker, silent fall-through.
func maybePromptForClonePAT(ctx context.Context, io IO, deps deps, circuit string, cmd *newCmd) error {
	owner, repo, ok := parseGitHubCloneOwnerRepo(cmd.Clone)
	if !ok {
		return nil
	}
	var matches []wire.PatResult
	if err := deps.call(ctx, circuit, wire.MethodPatFindForClone, wire.PatFindForCloneParams{Owner: owner, Repo: repo}, &matches); err != nil {
		// Non-fatal: if the registry call fails we just don't pop the
		// picker. The user can still pass --pat=<slug> by hand. Surface
		// the error on stderr so it isn't completely silent.
		fmt.Fprintf(io.Stderr, "warning: pat auto-detect failed: %v\n", err)
		return nil
	}
	if len(matches) == 0 {
		return nil
	...
	const skipSentinel = ""
	now := time.Now()
	opts := make([]huh.Option[string], 0, len(matches)+1)
	for _, m := range matches {
		opts = append(opts, huh.NewOption(formatPatPickerRow(m, now), m.Slug))
	}
	opts = append(opts, huh.NewOption("skip / no PAT for this kart", skipSentinel))

	pick := matches[0].Slug
	sel := huh.NewSelect[string]().
		Title(fmt.Sprintf("PATs covering %s/%s", owner, repo)).
		Description("Pick a registered PAT for this kart · enter to confirm · esc to skip").
		Options(opts...).
...
// formatPatPickerRow renders one row in the auto-detect picker. Layout:
//
//	<slug>   <owner>  <expires-or-no-expiry>  <name>
//
// Days-to-expiry annotates expires_at when it's set so the user spots
// soon-to-rotate tokens at a glance. The pat's name (the "display name"
// from the GitHub settings page) goes last because it's the loosest
// signal — the slug is the durable id.
func formatPatPickerRow(m wire.PatResult, now time.Time) string {
	expires := "no expiry"
	if m.Pat.ExpiresAt != "" {
		expires = m.Pat.ExpiresAt
		if t, err := time.Parse("2006-01-02", m.Pat.ExpiresAt); err == nil {
			days := int(t.Sub(now).Hours() / 24)
			switch {
			case days < 0:
				expires = fmt.Sprintf("%s (expired)", m.Pat.ExpiresAt)
			case days == 0:
				expires = fmt.Sprintf("%s (today)", m.Pat.ExpiresAt)
			default:
				expires = fmt.Sprintf("%s (%dd)", m.Pat.ExpiresAt, days)
			}
		}
	}
	owner := m.Pat.Owner
	...
	return fmt.Sprintf("%-24s  @%-20s  %-22s  %s", m.Slug, owner, expires, name)
}

// validatePatFlag rejects raw token literals on `--pat`. The flag is for
// registered slugs only — paste flow is `drift pat new`, not embedded in
// `drift new`. Empty value and the literal "none" pass through; anything
// that looks like a fine-grained or classic PAT is bounced with a hint.
func validatePatFlag(value string) error {
	v := strings.TrimSpace(value)
	if v == "" || v == "none" {
		return nil
	}
	low := strings.ToLower(v)
	if strings.HasPrefix(low, "github_pat_") || strings.HasPrefix(low, "ghp_") {
		return errors.New(
			"--pat takes a registered slug, not a raw token; register the token via `drift pat new`, then pass its slug")
	}
	return nil
}
...
// expandCloneShorthand lets `--clone owner/repo` stand in for the github
// HTTPS URL. Skips anything containing `:` (real URLs, scp-style ssh
// paths) or starting with `/`, `.`, `~` (filesystem paths) so we only
// rewrite what unambiguously looks like a github slug.
func expandCloneShorthand(cmd *newCmd) {
	if cmd.Clone == "" {
		return
	}
	if strings.ContainsAny(cmd.Clone, ":") {
		return
	}
	if strings.HasPrefix(cmd.Clone, "/") || strings.HasPrefix(cmd.Clone, ".") || strings.HasPrefix(cmd.Clone, "~") {
		return
	}
	parts := strings.Split(cmd.Clone, "/")
	if len(parts) != 2 {
		return
	}
	owner, repo := parts[0], parts[1]
