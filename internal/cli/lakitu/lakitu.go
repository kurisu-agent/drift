...
type CLI struct {
	...
	NixCache  nixCacheCmd  `cmd:"" name:"nix-cache" help:"Inspect this circuit's Nix binary cache (provided by services.lakitu.nixCache)."`
}
...
func Run(ctx context.Context, argv []string, io IO) int {
	...
	case "nix-cache info":
		return runNixCacheInfo(ctx, io, cli.NixCache.Info)
	default:
...
func registerDevpodBacked(ctx context.Context, reg *rpc.Registry, garage string, debug bool) {
	...
	kartDeps := server.KartDeps{
		...
		LoadCharacter: lifeDeps.LoadCharacterForInfo,
		LoadPAT:       lifeDeps.LoadPATForInfo,
	}
...
func methodNeedsDevpod(method string) bool {
	...
	case wire.MethodKartNew, wire.MethodKartStart, wire.MethodKartStop,
		...
		wire.MethodKartProbePorts, wire.MethodServerStatus:
		return true
	}
	return false
}
