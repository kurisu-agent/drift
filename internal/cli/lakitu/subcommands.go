...
type configSetCmd struct {
	Key   string `arg:"" help:"Dotted key: name, icon, default_tune, default_character, nix_cache_url, chest.backend."`
...
type characterNewCmd struct {
	...
	DisplayName string `name:"display-name" help:"Free-form name shown in the in-kart zellij topbar / claude-statusline. Falls back to the YAML key when unset."`
	Icon        string `name:"icon" help:"Nerd-font catalog name (e.g. dev-go), single grapheme, or emoji. Browse with 'drift circuit icons <q>'."`
	Color       string `name:"color" help:"Catppuccin-mocha palette name (green, mauve, peach, ...) tinting character-flavoured topbar segments."`
}
