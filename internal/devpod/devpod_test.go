...
func TestUpArgsPropagateAllFlags(t *testing.T) {
	...
	c := newClient(f)
	devpod.MarkContextOptionsAppliedForTest(c)
...
// TestEnsureContextOptionsFiresOnceBeforeFirstUp guards the gh-auth
// invariant: every devpod Client must disable
// SSH_INJECT_GIT_CREDENTIALS once before the first Up so the kart's
// per-character gitconfig and gh credential helper aren't overwritten
// by lakitu's host-level git credential store. Subsequent Ups must
// not re-fire.
func TestEnsureContextOptionsFiresOnceBeforeFirstUp(t *testing.T) {
	t.Parallel()
	f := &fakeRunner{replay: []fakeReply{
		{stdout: "ok"}, // context set-options
		{stdout: "ok"}, // first Up
		{stdout: "ok"}, // second Up (no second set-options)
	}}
	c := newClient(f)
	...
	if len(f.calls) != 3 {
		t.Fatalf("call count = %d, want 3 (one set-options + two ups)", len(f.calls))
	}
	wantFirst := []string{"context", "set-options", "-o", "SSH_INJECT_GIT_CREDENTIALS=false"}
	if diff := cmp.Diff(wantFirst, f.calls[0].Args); diff != "" {
		t.Errorf("first-call args mismatch (-want +got):\n%s", diff)
	}
	for i := 1; i < 3; i++ {
		if got := f.calls[i].Args[0]; got != "up" {
			t.Errorf("call %d: first arg = %q, want %q", i, got, "up")
		}
	}
}

func TestStopDeleteLogsHappyPath(t *testing.T) {
	t.Parallel()
