...
func RegisterServer(reg *rpc.Registry, d *Deps) {
	...
	reg.Register(wire.MethodCharacterNew, d.CharacterNewHandler)
	reg.Register(wire.MethodCharacterPatch, d.CharacterPatchHandler)
	reg.Register(wire.MethodCharacterReplace, d.CharacterReplaceHandler)
	...
	reg.Register(wire.MethodTuneNew, d.TuneNewHandler)
	reg.Register(wire.MethodTunePatch, d.TunePatchHandler)
	reg.Register(wire.MethodTuneReplace, d.TuneReplaceHandler)
	...
	reg.Register(wire.MethodChestNew, d.ChestNewHandler)
	reg.Register(wire.MethodChestPatch, d.ChestPatchHandler)
	...
	reg.Register(wire.MethodPatNew, d.PatNewHandler)
	reg.Register(wire.MethodPatUpdate, d.PatUpdateHandler)
	reg.Register(wire.MethodPatList, d.PatListHandler)
	reg.Register(wire.MethodPatShow, d.PatShowHandler)
	reg.Register(wire.MethodPatRemove, d.PatRemoveHandler)
	reg.Register(wire.MethodPatFindForClone, d.PatFindForCloneHandler)
...
// LoadCharacterForInfo is the exported adapter kart.info binds to
// KartDeps.LoadCharacter so it can pull display_name / icon / color
// for the operator-facing single-kart view. Thin wrapper around
// loadCharacter to keep the unexported helper unexported.
func (d *Deps) LoadCharacterForInfo(name string) (*Character, error) {
	return d.loadCharacter(name)
}

// LoadPATForInfo is the exported adapter kart.info / kart.list bind to
// KartDeps.LoadPAT so they can attach expiry metadata to each kart's
// response. Thin wrapper around loadPat. Returns the same pat_not_found
// rpcerr loadPat does so callers can distinguish "slug missing" from
// "I/O failure" — kart.info treats pat_not_found as "Missing: true" and
// keeps rendering rather than aborting.
func (d *Deps) LoadPATForInfo(slug string) (*pat.Pat, error) {
	return d.loadPat(slug)
}
