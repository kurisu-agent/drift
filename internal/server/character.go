...
// `chest:<name>` reference — literal tokens are rejected at new/patch
// time. DisplayName / Icon / Color are presentation only; see model.Character.
...
func (d *Deps) CharacterNewHandler(_ context.Context, params json.RawMessage) (any, error) {
	...
	if err := icons.Validate(p.Icon); err != nil {
		return nil, rpcerr.UserError(rpcerr.TypeInvalidFlag, "character.new: %v", err)
	}
...
func validateCharacter(c *Character) error {
	...
	if err := icons.Validate(c.Icon); err != nil {
		return rpcerr.UserError(rpcerr.TypeInvalidFlag, "character: %v", err)
	}
	return nil
}
